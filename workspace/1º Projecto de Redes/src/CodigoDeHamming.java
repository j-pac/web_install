...
public class CodigoDeHamming extends TecnicaControloErros {
	...
	public boolean deteccaoErros(int[] trama_recebida) {
		check_bits_sum = 0;
		// Ciclo que verifica o valor dos check bits e, em caso de assumirem o
		// valor 1, adiciona-os à posicao do erro
		for (int i = 0; (int) Math.pow(2, i) < trama_recebida.length; i++) {
			if (XOR(trama_recebida[(int) Math.pow(2, i) - 1],
					calcularBitParidade((int) Math.pow(2, i), trama_recebida)) == 1) {
				check_bits_sum += (int) Math.pow(2, i);
			}
		}
		return check_bits_sum != 0;
	}
